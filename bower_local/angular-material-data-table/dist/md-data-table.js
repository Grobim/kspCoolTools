...
function mdColumnHeader($compile, $interpolate, $timeout, $injector) {
  'use strict';
  ...
  function postLink(scope, element, attrs, ctrls) {
    ...
    var hasAngularTranslate = $injector.has('$translate');
    var name;

    if (scope.nameKey && !hasAngularTranslate) {
      console.warn('name-key argument provided but couldn\'t find angular-translate');
    }

    if (scope.nameKey && hasAngularTranslate) {
      name = $interpolate.startSymbol() + 'nameKey | translate' + $interpolate.endSymbol();
    } else {
      name = $interpolate.startSymbol() + 'name' + $interpolate.endSymbol();
    }
    ...
    if(attrs.orderBy) {
      ...
      if (angular.isDefined(attrs.orderByStatic)) {
        sortIcon = angular.element('<ng-md-icon></ng-md-icon>');
      } else {
        sortIcon = angular.element('<md-icon></md-icon>');
      }
      ...
      scope.getIcon = function() {
        if(isActive()) {
          return headCtrl.order[0] === '-' ? 'keyboard_arrow_down' : 'keyboard_arrow_up';
        }
        return angular.isDefined(attrs.descendFirst) ? 'keyboard_arrow_down' : 'keyboard_arrow_up';
      };
      
      if (angular.isDefined(attrs.orderByStatic)) {
        sortIcon.attr('icon', '{{getIcon()}}');
      } else {
...
mdColumnHeader.$inject = ['$compile', '$interpolate', '$timeout', '$injector'];
